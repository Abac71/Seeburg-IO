...
#define IGNORE_CHANGE_BELOW_USEC 58000 //change_abac71 from 0.01 to 0.05 sec
// What is the minimum time since the last pulse for a pulse to count as "after the gap"
#define MIN_GAP_LEN_USEC 230000 //change_abac71 from 0.25 to 0.24 sec
...
int main(int argc, char **argv) {
	...
	for (;;) {
		...
		// Calc the key combination... (for a 3wA pulse train max_post = 10 and max_pre = 21)
				if (pre >= 1 && pre <= 13) {							//insert_abac71	
					number = post;								//insert_abac71	
					letter = 'A' + (pre - 1);						//insert_abac71	
					}									//insert_abac71	
				else if (pre >= 14 && pre <= 21) {						//insert_abac71	
					number = post;								//insert_abac71	
					letter = 'A' + (pre);							//insert_abac71	
					}									//insert_abac71	
